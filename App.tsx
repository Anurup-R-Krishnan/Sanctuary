...
const App: React.FC = () => {
  ...
  const { books, addBook, updateBookProgress, isLoading: isLibraryLoading } = useBookLibrary({ persistent: libraryPersistenceEnabled });
  ...
  const handleUpdateGoal = useCallback((daily: number, weekly: number) => {
    setDailyGoal(daily);
