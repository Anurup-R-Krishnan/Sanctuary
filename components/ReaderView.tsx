...
const ReaderView: React.FC<ReaderViewProps> = ({
  ...
  useEffect(() => {
    ...
    rendition
      .display(book.lastLocation || undefined)
  ...
  return (
    <div className="fixed inset-0 bg-light-primary dark:bg-dark-primary z-[60] animate-[fadeIn_0.5s_ease-out]">
      {/* Header Controls */}
      <div
        className={`fixed top-0 left-0 right-0 p-4 z-20 transition-all duration-300 ${showControls ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-full"}`}
      >
      ...
      >
        ...
        <div
          id="viewer"
          ref={viewerRef}
          className={`w-full h-full max-w-4xl reader-content transition-opacity duration-300 ${isLoading ? "opacity-0" : "opacity-100"}`}
        ></div>
      </div>
      <div
        onClick={prevPage}
      ...
      {/* Footer Controls */}
      <div
        className={`fixed bottom-0 left-0 right-0 p-4 z-20 transition-all duration-300 ${showControls ? "opacity-100 translate-y-0" : "opacity-0 translate-y-full"}`}
      >
